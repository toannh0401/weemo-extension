...
SightCallExtension.prototype.checkCallOneOne = function() {
  ...
  jqchat.ajax(callOneOneUrl)
    .done(function(meetingId) {
      if (meetingId !== "") {
        ...
        jqchat.ajax("/rest/weemo/finishOneOne/" + sightcallExtension.username)
          .done(function() {
  ...
  setTimeout(function() {
    sightcallExtension.checkCallOneOne()
  }, 500);
};
...
(function($) {
  ...
  $(document).ready(function() {
    ...
    window.require(["SHARED/SightCallNotification"], function(sightCallNotification) {
      sightCallNotification.initCometd(sightcallExtension.username, sightcallExtension.cometdUserToken, sightcallExtension.cometdContextName);
    });
  });
